...
<!--                   <button type="button" data-action="add" class="action btn btn-sm waves-effect btn-primary m-b-5 tooltip-hover pull-right submit btn-modal" title="Tambah Retribusi Nilai Tarif"> 
                    <i class="fa fa-plus"></i> Tambah Retribusi Nilai Tarif
                  </button> -->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Retribusi Nilai Tarif</h4>
            </div>
...
<script type="text/javascript">
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('admin/show_dm_tarif')?>";
  var   totalClmn;
  var   table1;
  ...
  function show_modal(data) {
    ...
    if (mode == "add") {
      $('#form-field').children('.modal').find('.modal-title').text("Tambah Retribusi Nilai Tarif");
      $('#form-field').children('.modal').modal('show');
        
    } else if (mode == "edit") {
      $.ajax({
        url : "<?php echo base_url('admin/dm_tarif_action')?>/"+"detail",
        dataType: "json",
      ...
      $('#form-field').children('.modal').find('.modal-title').text("Ubah Retribusi Nilai Tarif");
      $('#form-field').children('.modal').modal('show');
  ...
  $(document).on('click', ".action", function() {
        ...
        if (action == "aktif") {
            ...
            $.ajax({
              url : "<?php echo base_url('admin/dm_tarif_action')?>/"+action,
              dataType: "json",
          ...
          $.ajax({
            url : "<?php echo base_url('admin/dm_tarif_action')?>/"+mode,
            dataType: "json",
            data: dataForm,
            type: "POST",
            success: function(data) {
