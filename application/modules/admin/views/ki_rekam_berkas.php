      ...
      <div class="row">
          ...
          <div class="col-sm-9">
              <div class="card-box table-responsive">
                  ...
                  <button type="button" data-action="add" class="action btn btn-sm waves-effect btn-primary m-b-5 tooltip-hover pull-right submit btn-modal" title="Tambah Grup Berkas"> 
                    <i class="fa fa-plus"></i> Tambah Grup Berkas
                  </button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Grup Berkas</h4>
            </div>
            <div class="modal-body">
                <input name="id_rekam_berkas_grup" type="hidden">
  ...
  <div class="modal fade" tabindex="-1" role="dialog" >
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input name="id_rekam_berkas_s" type="hidden">
...
<script type="text/javascript">
  ...
  function set_title() { 
    $.ajax({
      url : "<?php echo base_url('admin/ki_rekam_berkas_action')?>/"+"get_jenis_izin",
      dataType: "json",
  ...
  // My Datatables
  var   urlDb   = "<?php echo base_url('admin/show_ki_rekam_berkas_grup')?>";
  var   totalClmn;
  var   table1;
  ...
  function get_datag() {
    ...
    $.ajax({
      url : "<?php echo base_url('admin/ki_rekam_berkas_action')?>/"+"get_dt_group",
      dataType: "json",
      data: 'jenis_izin='+jenis_izin,
      type: "POST",
      success: function(data) {
  ...
  function show_modal(data) {
    ...
    if (mode == "add") {
      $('#form-field-grup').children('.modal').find('.modal-title').text("Tambah Grup Syarat");
      $('#form-field-grup').children('.modal').modal('show');
        
    } else if (mode == "edit") {
      $.ajax({
        url : "<?php echo base_url('admin/ki_rekam_berkas_action')?>/"+"detail",
        dataType: "json",
        data: "id_rekam_berkas_grup="+data,
        type: "POST",
        success: function(data) {
          $('#form-field-grup').find('input[name="id_rekam_berkas_grup"]').val(data.id_rekam_berkas_grup);
      ...
      $('#form-field-grup').children('.modal').find('.modal-title').text("Ubah Grup Syarat");
      $('#form-field-grup').children('.modal').modal('show'); 
    } else if (mode == "add-child") {
      $('#form-field-child').children('.modal').find('.modal-title').text("Tambah Data Syarat");
      $('#form-field-child').children('.modal').modal('show'); 
    } else if (mode == "edit-child") {
      $.ajax({
        url : "<?php echo base_url('admin/ki_rekam_berkas_action')?>/"+"detail-child",
        dataType: "json",
        data: "id_rekam_berkas_s="+data,
        type: "POST",
        success: function(data) {   
          $('#form-field-child').find('input[name="id_rekam_berkas_s"]').val(data.id_rekam_berkas_s);
          $('#form-field-child').find('input[name="id_rekam_berkas_grup"]').val(data.id_rekam_berkas_grup);
      ...
      $('#form-field-child').children('.modal').find('.modal-title').text("Ubah Data Syarat");
      $('#form-field-child').children('.modal').modal('show'); 
    }     
  }
  ...
  var datatables_c = $.ajax({
      url : "<?php echo base_url('admin/ki_rekam_berkas_action')?>/"+"get_dt_child",
      dataType: "json",
      type: "POST"
    })
  ...
  $(document).on('click', ".action", function() {
        ...
        if (action == "show-group") {
          ...
          $.ajax({
            url : "<?php echo base_url('admin/ki_rekam_berkas_action')?>/"+mode,
            dataType: "json",
            data: dataForm+"&id_jenis_izin="+jenis_izin,
            type: "POST",
            success: function(data) {
                swal({
                  title: "Berhasil!",
                  text: 'Grup Syarat telah disimpan',
                  type: "success",
                });
                refresh_group();
            }
          ...
          $.ajax({
            ...
            success: function(data) {
                swal({
                  title: "Berhasil!",
                  text: 'Data Syarat telah disimpan',
                  type: "success",
                });
                refresh_child();
            }
            ...
            var rekam_berkas_s= $(this).attr('data-id');
            var nama        = $(this).closest("tr").find("td:eq(1)").text();

            $.ajax({
              url : "<?php echo base_url('admin/ki_rekam_berkas_action')?>/"+action,
              dataType: "json",
              data: 'rekam_berkas_s='+rekam_berkas_s+'&status='+status,
              type: "POST",
              success: function(data) {
                if (data.status == 1) {
                  var text    = 'aktifkan';
