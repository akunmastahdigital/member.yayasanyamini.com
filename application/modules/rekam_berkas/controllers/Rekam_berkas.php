...
class Rekam_berkas extends MY_Controller {
	...
	function rekam_berkas_detail($permission, $id_permohonan) {
		$d['page']      	= 'rekam_berkas_detail';
		$d['menu']      	= 'Detail Rekam Berkas';
		$d['title']      	= 'Detail Rekam Berkas';
			
		$condition 			= [];
		$condition[]  		= ['id_permohonan', $id_permohonan, 'where'];
		$id_jenis_izin 	    = $this->M_core->get_tbl('t_permohonan', 'id_jenis_izin', $condition)->row_array()['id_jenis_izin'];

		//rekam berkas
		$condition 	  = [];
		$condition[]  = ['aktif', 1, 'where'];
        $condition[]  = ['id_jenis_izin', $id_jenis_izin, 'where'];
		$condition[]  = ['kd_rekam_berkas_grup', 'asc', 'order_by'];
		$dRb['id_permohonan'] = $id_permohonan;	
		$dRb['id_jenis_izin'] = $id_jenis_izin;	
		$dRb['permission']    = $permission; 
		$dRb['m_grup'] 	      = $this->M_core->get_tbl('m_rekam_berkas_grup', '*', $condition)->result_array();
		$dRb['rb_card'] 	  = $this->load->view('rekam_berkas_card', $dRb, true);

		$d['rekam_berkas']    = $dRb['rb_card'];
		
		$this->load->view('layout', $d);
	}

	function rekam_berkas_submit() {
		$id_permohonan  = $this->input->post('id_permohonan');
		$id_jenis_izin  = $this->input->post('id_jenis_izin');
		$permission  	= $this->input->post('permission');

		//submit rekam berkas
		$core_rb		= 'rekam_berkas';
		$smbt_rb		= $this->process_all($core_rb, $id_jenis_izin, $id_permohonan);

		$this->session->set_flashdata("tipe-alert", "success");
		$this->session->set_flashdata("teks-alert", "Data rekam berkas berhasil disimpan!");
		$this->session->set_flashdata("icon-alert", "fa fa-check-circle");
		// redirect('rekam_berkas/rekam_berkas_detail/'.$permission.'/'.$id_permohonan);
